# 👋 Hi, I'm Leo / レオです

**🇬🇧 AI-native, product-minded full-stack developer seeking opportunities in Japan. I ship modern web apps and AI agents — React, Angular, Next.js, Spring Boot.**  
**🇯🇵 日本の技術業界で働くことを夢見ている、プロダクト志向のフルスタック開発者です。AIエージェントとモダンなWebアプリをつくっています。**

🌏 Tokyo-focused · 🤖 From winning hackathons to building AI agents

💼 [LinkedIn](https://www.linkedin.com/in/leo-paul-martin) · 🌐 [Portfolio](https://leopaul29.github.io) · ✍️ [Blog](https://lpm.hashnode.dev/)

---

## 🚀 Featured Projects / 注目プロジェクト

_Selected from 20+ hackathons — prize-winning, AI-focused, Tokyo-built._

**🥈 [AI Bicycle](https://github.com/leopaul29/AI-Bicycle-Liquid-AI-Tokyo-Hackathon)** — Liquid AI × W&B × Lambda Hackathon · **2nd Place**  
A playful AI companion inspired by a mascot train from a Japanese anime.

**🏅 [AI Reminder Bot](https://github.com/leopaul29/waytoagi-25102025)** — Wayto AGI · **Special Prize** · [📰 write-up](https://lpm.hashnode.dev/waytoagi-25102025)  
An AI-powered reminder & to-do assistant.

**🤖 [Kizuna Link](https://github.com/leopaul29/impacttokyo-07032026)** — Impact Tokyo  
Voice-first AI companion connecting seniors to city-hall services and their family.  
`Next.js` · `OpenAI API` · `LINE Messaging API`

👉 [See all 20+ hackathons](./hackathons.md)

---

## 🛠 Tech Stack / 技術スタック

![myskills](https://skillicons.dev/icons?i=ts,react,angular,nextjs,tailwind,nodejs,java,spring,py,pytorch,solidity,postgres,docker,azure,githubactions,bitbucket&theme=light)
